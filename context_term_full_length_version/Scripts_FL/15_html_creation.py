...
file_path = os.path.join(base_dir, '/app/full_length_version/Outputs_FL/14_fl_dis_spec_with_missing_rows.xlsx')
data = pd.read_excel(file_path)

# Ensure 'pmcid' and 'section' are treated as strings
data['pmcid'] = data['pmcid'].astype(str)
...
# Process each group by pmcid and section
for pmcid, pmcid_group in data.groupby('pmcid'):
    for section, group in pmcid_group.groupby('section'):
        # Combine pmcid and section for the group name
        pmcid_section = f"{pmcid}_{section}"
        html_content += f"<h2>{pmcid_section}</h2>\n"
        sentences_by_index = group.groupby('sent_index')

        # Process each sentence within the pmcid_section
...
output_html_path = os.path.join(base_dir, '/app/full_length_version/Outputs_FL/highlighted_entities_FL.html')
